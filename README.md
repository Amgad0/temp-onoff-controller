...
2. **Uninitialized `setpoint` variable.** In `Main_Task`, `unsigned char setpoint` is never initialized and is only ever written by a non-blocking queue read — before the user's first UART entry, the heater/LED logic runs against whatever garbage happens to be on the stack.
3. **Operator-precedence bug in the temperature conversion.** `Temperature=(int) mV/10.0;` casts `mV` to `int` *before* dividing (the cast binds tighter than `/`), truncating a step earlier than intended.
4. **LCD peripheral-clock race.** `LCD_setup()` enables the Port A/C clocks and configures their pins on the very next line, with no wait for the clock to stabilize — a known TM4C123 gotcha.
5. **Dead/duplicate entry points and stale names.** `Trial.c` is an unused standalone LCD test; `Main0.c` (the real entry point) and `Trial2.c`/`Trial2.h` (the application core) carry legacy names from earlier demo iterations, and the Keil project itself is still named `Lab3`.
6. **Monolithic application file.** `Trial2.c` holds ADC/UART/GPIO init *and* all four RTOS tasks in one file.
7. **Avoid busy-waiting inside RTOS tasks.** `Main_Task` and `Buzzer_Task` never call `vTaskDelay`, and every queue read uses a `0` (non-blocking) timeout. This only works because `configUSE_TIME_SLICING` is enabled — each task still burns its full time slice spinning rather than yielding.
8. **Guard the LCD text buffers.** `Message.Txt1`/`Txt2` are fixed 4-byte arrays fed by `itoa`; a 3-digit temperature plus the null terminator exactly fills the buffer with zero margin for a sign character or a 4th digit.
9. **Validate UART input.** `UART_Task` accumulates every received byte as `N - '0'` with no check that the character was actually a digit, and no bound on the number of digits entered before Enter.
